...
def data_prepare_play(X, train_set=True):
    slp_ = X['slp']
    t_slp_ = X['t_slp']
    ...
    if train_set == True:
        ...
        Y_tr = np.array([Y_1_train, Y_2_train])
        Y_val = np.array([Y_1_val, Y_2_val])



        train_data = list(zip(np.array([pressure1_train, pressure2_train]), np.array([surge1_train, surge2_train]), 
        np.array([t_surge1_train, t_surge2_train]), Y_tr))
        val_data = list(zip(np.array([pressure1_val, pressure2_val]), np.array([surge1_val, surge2_val]), 
        np.array([t_surge1_val, t_surge2_val]), Y_tr))
        batch_size = 8
